...
ggline(HazingSurveyData, x = "BinGlty", y = "Score", 
    ...
    add.params = list(alpha = 0.5))
      ...
      HazingSurveyData %>% 
        ...
        formatRound(columns = c("W Stat", "p.value",
                                "mean", "sd",
                                "stderr", "LCL", "UCL"), digits = 4)  
      ...
      tidy(wilcox.test(Score ~ BinGlty, HazingSurveyData, na.rm = TRUE, 
                       paired = FALSE, exact = FALSE, conf.int = TRUE)) %>% 
        datatable(options = list(dom = 't')) %>% 
        formatRound(columns = c("estimate", "p.value",
                                "conf.low", "conf.high"), digits = 4)
  
      
      tidy(wilcox.test(Score ~ Gender, HazingSurveyData, na.rm = TRUE, 
                       paired = FALSE, exact = FALSE, conf.int = TRUE)) %>% 
        datatable(options = list(dom = 't')) %>% 
        formatRound(columns = c("estimate", "p.value",
                                "conf.low", "conf.high"), digits = 4)
